...
func _prepare_set_shortcut(shortcut_data: ShortcutData) -> void:
	if shortcut_data:
...
func _on_shortcut_clicked(action_name, shortcut) -> void:
	if _prepare_shortcut_data:
		...
		# Consider if we should not delegate this to the Shortcut node itself
		_prepare_shortcut_data.slot = action_name
