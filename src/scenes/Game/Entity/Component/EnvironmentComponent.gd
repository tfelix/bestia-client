...
func on_update(entity, new_component) -> void:
	...
	GlobalEvents.emit_signal("onTemperatureChanged", temp_data)
	...
	GlobalEvents.emit_signal("onWeatherChanged", weather_data)
	
	var time_data = DayData.new()
	time_data.sunrise = sunrise
	time_data.sunset = sunset
	time_data.progress = day_progress
	GlobalEvents.emit_signal("onDaytimeChanged", time_data)
